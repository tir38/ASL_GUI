function varargout = ASL_GUI__RECOGNIZE(varargin)
...
%             trackedImage=image2; %jvf2
            ...
            left = ceil(x0/scl);
            top = ceil(y0/scl);
            right = floor((x0 + W)/scl);
            bottom = floor((y0 + H)/scl);
            
% %             plot(left,top,'ro');
            line([left, left],[top,bottom])
            line([left, right],[bottom,bottom])
            line([right, right],[top,bottom])
            line([left, right],[top,top])
            hold off
            ...
            if mTimer == 0
                ...
                subplot(1,2,2)
                imshow(predictImage)
                title ('exact image sent to SIFT')
                hold off

...
T = rgb2ind(T,map);
    figure(6)
    subplot(2,2,3)
    imshow(T, [min(min(T)), max(max(T))])
    title('template in color index')
    hold off

% Estimation of the target PDF
q = Density_estim(T,Lmap,k,H,W,1);
...
for t=1:Length-1
    ...
    [x,y,loss,f,f_indx,T2,p] = MeanShift_Tracking(q,I2,Lmap,...
    ...
    % use current T2 for next iteration
    q = p;
    T = T2;
end
fprintf('just computed at %f  Hz.\n',Length/toc)
close(WaitBar);
%%%% End of TRACKING
